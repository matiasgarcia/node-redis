...
function forwardWrite(val: Buffer | string) {
  replicaConnections.forEach((replicaConnection) => {
    console.debug(`<<[fwd][${config.role}][${[replicaConnection.remoteAddress, replicaConnection.remotePort].join(':')}] ${Utils.loggableBuffer(val)}`);
...
function processCommand(stream: string, connection: net.Socket) {
  ...
  console.debug(`>>[${config.role}][${host}:${config.port}] ${Utils.loggableBuffer(stream)}`);
  if (tokens.length === 0) {
    return;
  }
