...
function receiveCommands(stream: Buffer, connection: net.Socket) {
...
net
.createServer((client) => {
  client.on('data', (buffer) => receiveCommands(buffer, client))
})
...
.on('listening', async () => {
  ...
  master.on('data', (buffer) => receiveCommands(buffer, master))
})
