...
function parseDatabaseSection(databaseSection) {
    ...
    if (databaseSection[1] !== HASH_TABLE_SECTION_START)
        throw new Error('Missing hash table');
...
export function readRdbFile(rdbFileDir, dbFileName) {
    ...
    if (version !== 'REDIS0011')
        throw new Error('Unexpected redis version');
    if (rdb[8] === METADATA_START)
